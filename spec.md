...
- 발송 실패 시 지수 백오프로 자동 재시도. 기본: 최대 3회, 간격 30초 → 2분 → 10분 (설정화)
...
- 수신 불가 수신자(탈퇴 등)를 시뮬레이션할 수 있어야 한다
  (예: `withdrawn-*` 패턴 → 수신 불가, §7.7).

**AC**
- [ ] 실패 주입 규칙으로 FR-5의 시나리오를 통합 테스트에서 재현 가능
- [ ] 수신 불가 수신자 → 재시도 없이 FAILED + 실패 코드 `RECIPIENT_GONE` (§7.7)
...
> 선택 구현 중 질문형 항목(읽음 동시 처리, 재시도 횟수 초기화)은 구현 여부와
> 무관하게 정책 서술을 README에 반드시 포함한다.
...
- **키 정의**: `idempotency_key = type + ":" + refType + ":" + refId + ":" + receiverId + ":" + channel`
...
- **중복 응답 정책**: 오류(409)가 아닌 `200 + 기존 알림 ID`.
  근거: 멱등의 정의는 "같은 요청은 몇 번 보내도 같은 결과"이며, 재시도하는
  호출자에게 409는 정상 재시도와 실제 오류를 구분시키는 부담을 지운다.
...
| RecipientStatusPort (발송 직전 수신 가능 검증) | 패턴 기반 스텁 (`withdrawn-*` = 수신 불가) | 사용자 서비스 조회 또는 사용자 상태 이벤트 프로젝션 |
...
원 요구사항이 열어둔 부분에 대한 본 구현의 결정이다. 전부 README의
"요구사항 해석 및 가정"에 옮겨 기술한다.
...
- 잔여 위험과 근본 해결 방향(수신측 멱등성, 발송 게이트웨이의 dedupe)은 README
  개선 의견에 기술한다.

### 7.5 수동 재시도 시 횟수 초기화 (선택 구현 질문)

**초기화한다.** 수동 재시도는 운영자가 원인(예: 외부 서버 복구)을 확인하고 내리는
새로운 처리 지시이므로, 소진된 자동 재시도 예산과 별개의 예산을 갖는 것이 맞다.
단, 기존 시도 이력은 보존되어 전체 이력이 추적 가능해야 한다.

### 7.6 알림 보관

SENT/FAILED 알림은 삭제하지 않고 보존한다(사용자 목록 조회 + 운영 추적의 원천).
대용량 환경에서의 아카이빙 정책은 범위 외로 하되 README 개선 의견에 언급한다.
...
데이터 파기(또는 비식별화)가 필요함을 README 개선 의견에 기술한다.
...
**본 과제에서의 구현**
- 사용자 도메인이 존재하지 않으므로 검증 지점만 포트(`RecipientStatusPort`)로
  드러내고, 스텁 구현은 수신자 ID 패턴(`withdrawn-*` = 수신 불가)으로 시뮬레이션한다.
- 워커는 발송 직전에 이 포트를 호출하며, 수신 불가 판정 시 **재시도 없이
  FAILED + 실패 코드 `RECIPIENT_GONE`** 으로 종료한다(§7.3의 영구적 실패로 분류).
- 별도 CANCELED 상태를 도입하지 않는다 — 상태 머신 단순화를 우선하며, "실패가
  아니라 발송 사유 소멸"을 구분하는 CANCELED 도입과 탈퇴 이벤트 구독에 의한
  선제 취소는 실운영 개선안으로 README에 기술한다.
...
| NFR-4 | 스케줄러 분리 | 폴러와 스턱 회수 스케줄러가 상호 블로킹하지 않도록 스케줄러/워커 스레드풀 분리 |
| NFR-5 | 관측성 | 상태 전이·시도 결과를 구조화 로그로 남김. 실패 사유는 반드시 DB에 기록 |
...
공통: 사용자 식별은 `X-USER-ID` 헤더로 간략 처리 (과제 공통 제약).
에러 응답은 `{ "code": "...", "message": "..." }` 형식으로 통일.

| # | 메서드 | 경로 | 설명 | 응답 |
| --- | --- | --- | --- | --- |
| 1 | POST | `/api/notifications` | 발송 요청 등록 | 202 (신규) / 200 (중복, 기존 ID) / 400 |
...
| 5 | POST | `/api/notifications/{id}/retry` | 최종 실패 수동 재시도 (OPT-1) | 200 / 404 / 409(FAILED 아님) |
...
| R-4 | `@Scheduled` 기본 단일 스레드로 폴러·회수 상호 블로킹 | 처리 지연 | 스케줄러 스레드풀 크기 설정 + 발송은 별도 워커 풀로 위임 |
...
| 5 | 문서 (README 템플릿 9항목 + 비동기 구조/재시도 정책 문서 + 요구사항 해석/개선 의견 + AI 활용 범위), 최종 실행 검증 | clone → `docker compose up` → 데모 절차 재현 성공 |
...
- [ ] README.md — 템플릿 9개 항목 전부 포함
- [ ] 소스 코드 및 테스트 코드 — §4의 AC가 테스트로 존재
- [ ] 실행 방법 — Docker Compose 기준
- [ ] API 명세 또는 샘플 요청/응답 — §9 확장
- [ ] DB 스키마 또는 ERD 설명 — §5.5 확장
- [ ] AI 사용 내역 — 사용 범위와 직접 검증/수정한 내용을 구체적으로 기재
- [ ] **[C 전용] 비동기 처리 구조 및 재시도 정책 설명 문서** — §5, §6, FR-5를 README 섹션으로 정리
- [ ] **[C 전용] 요구사항 해석 및 개선 의견** — §7 전체 + 요구사항 자체에 대한 개선 제안
