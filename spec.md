...
**AC**
- [x] 유효한 요청 → 202 응답, DB에 PENDING 상태 알림 1건 생성
- [x] 응답 시점에 발송이 수행되지 않았음 (발송은 워커가 수행)
- [x] 필수 필드 누락/잘못된 enum 값 → 400 + 일관된 에러 응답 형식
- [x] 동일 멱등성 키 재요청 → 202 + 기존 알림 ID + `duplicated: true`, 신규 행 미생성
...
**AC**
- [x] 각 상태(PENDING/PROCESSING/SENT/FAILED)의 알림이 정확한 상태 값으로 조회됨
- [x] 실패 이력이 있는 알림 조회 시 실패 사유가 포함됨

### FR-3. 사용자 알림 목록 조회

- 수신자 기준으로 발송이 완료된 `SENT` 목록 조회. 읽음/안읽음 필터, 페이지네이션 지원
- 읽음 개념은 `IN_APP` 채널에만 적용 (§7.2)

**AC**
- [x] 수신자별 SENT 목록이 최신순으로 페이지네이션되어 반환됨
- [x] `read=true|false` 필터가 IN_APP 알림에 대해 동작함
...
**AC**
- [x] 정의되지 않은 전이 시도(예: SENT → PENDING)는 실패하며 상태가 변하지 않음
- [x] 모든 전이는 원자적으로 수행됨 (경쟁 상태에서 이중 전이 없음)
...
**AC**
- [x] 2회 실패 후 3회차 성공 시나리오: 최종 SENT, 시도 이력 3건 기록
- [x] 최대 횟수 연속 실패: FAILED 전환, last_error 및 전체 시도 이력 보존
- [x] 재시도 간격이 정책대로 스케줄됨 (next_attempt_at 검증)
- [x] 재시도 정책 수치가 설정 파일로 변경 가능함
...
**AC**
- [x] 동일 멱등성 키로 N개 스레드 동시 POST → 알림 정확히 1건 생성, 나머지는 기존 ID 반환
- [x] 애플리케이션 사전 검증을 우회한 경쟁 삽입이 DB UNIQUE 제약으로 차단됨
...
**AC**
- [x] PENDING 알림이 폴링 주기 내에 워커에 의해 처리됨 (Awaitility 검증)
- [x] 발송 지연(느린 Mock) 상황에서 API 응답 시간이 영향받지 않음
...
**AC**
- [x] processing_started_at이 임계 초과인 PROCESSING 행이 회수되어 재처리됨
- [x] PENDING 데이터 존재 상태에서 애플리케이션 재기동 → 자동 재처리
- [x] PENDING 100건 + 워커 4개 동시 폴링 → 각 알림이 동시에 중복 클레임되지 않고 누락 없이 처리됨
...
**AC**
- [x] 실패 주입 규칙으로 FR-5의 시나리오를 통합 테스트에서 재현 가능
- [x] 탈퇴 수신자 → 발송기 미호출, 재시도 없이 FAILED + `RECIPIENT_GONE` (§7.7)
- [x] 미존재 수신자 → 발송기 미호출, 재시도 없이 FAILED + `RECIPIENT_NOT_FOUND` + 경고 로그 (§7.7)
- [x] README에 실패 → 재시도 → 성공 흐름의 실행 가능한 데모 절차 포함
...
| PROCESSING → PENDING | 스턱 회수 | processing_started_at < now − 임계, attempt_count < max_attempts | attempt_count 유지, 회수 사유 기록 |
| PROCESSING → FAILED | 마지막 시도 스턱 회수 | processing_started_at < now − 임계, attempt_count ≥ max_attempts | 회수 사유를 최종 실패로 기록 |
...
SENT/FAILED 알림은 삭제하지 않고 보존한다. 사용자 목록에는 SENT만 노출하고,
FAILED는 단건 상태 조회와 운영 추적의 원천으로 사용한다.
...
- [x] Git repository URL — 커밋 히스토리가 작업 단위로 남을 것 (마일스톤 단위 이상으로 쪼갬)
