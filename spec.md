...
- **논리 키 정의**: `type:refType:refId:receiverId:channel`
  - 클라이언트가 `Idempotency-Key` 헤더를 명시하면 그 값을 논리 키로 우선 사용한다.
- **저장 키**: 논리 키의 **SHA-256 해시(hex, 64자)**. 원시 조합은 필드 최대 길이 합이
  컬럼(VARCHAR 200)을 넘길 수 있고(refId 100 + receiverId 50 + refType 50 + … ≈ 230자)
  구분자 충돌 위험도 있어, 길이 고정·충돌 안전을 위해 해시로 저장한다. 개별 필드는
  엔티티 컬럼에 그대로 남아 디버깅 시 재구성 가능하다.
- **방어 계층 (2중)**
