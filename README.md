...
발송이 완료된 `SENT` 알림만 최신순으로 반환하며 `size`는 1~100입니다.
...
발송이 완료된 SENT 상태의 IN_APP 알림만 지원합니다. 여러 기기가 동시에 요청해도 최초
요청만 `read_at`을 기록하며 이후 요청도 `200 OK`를 받습니다. 미발송 알림은
`409 NOTIFICATION_NOT_DELIVERED`, EMAIL 알림은 `400 CHANNEL_NOT_SUPPORTED`입니다.
...
| 409 | `NOTIFICATION_NOT_DELIVERED` | 아직 SENT가 아닌 알림의 읽음 처리 요청 |
...
stateDiagram-v2
    ...
    PROCESSING --> PENDING: retryable failure / recoverable stuck
    PROCESSING --> FAILED: permanent failure / retry exhausted / final-attempt stuck
```
...
- PROCESSING이 기본 5분을 넘으면 stuck recovery가 남은 시도가 있을 때만 PENDING으로 회수
- stuck 시도가 최대 시도 횟수에 도달했다면 추가 발송 없이 FAILED로 종료
...
- 미발송 알림 읽음 거부와 10개 동시 읽음 요청의 멱등성
